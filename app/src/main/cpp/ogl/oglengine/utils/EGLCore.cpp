...
void EGLCore::processTouchEvent(int actionMasked, int pointerCount,
                                 const float* xs, const float* ys, const int* ids) {
    pthread_mutex_lock(&mutex);
    TouchEvent ev;
    ev.actionMasked = actionMasked;
    ev.pointerCount = pointerCount > 10 ? 10 : pointerCount;
...
void EGLCore::requestReConnectSurface(JNIEnv* env, jobject surface) {
    ...
    // 删除旧的引用（DeleteGlobalRef 无线程限制，可用任意 env）
...
void EGLCore::renderLoopInternal() {
    ...
    while (running) {
        ...
        // 检查暂停状态
        if (paused) {
            // 即使处于暂停状态，也需要处理 Surface reconnect 请求
            // （当 app 回前台时，Surface 重建，paused 可能在 resume 之前被清除）
...
bool EGLCore::initEGL(JNIEnv* env) {
    ...
    // Scene should have been set via setScene() before init().
    // initEGL is called from within initSequential(), which runs after
    // setScene(). We now finalize scene setup.
    if (!scene) {
        LOGE("No scene set! Call setScene() before init().");
        return false;
    }

...
LitMaterial::LitMaterial() {
    ...
    lightPos[0] = lightPos[1] = lightPos[2] = 5.0f;
    lightColor[0] = lightColor[1] = lightColor[2] = 10.0f;
...
void LitMaterial::init() {
    ...
    LOGI("LitMaterial init done, %zu variants", variants.size());
}
...
LitMaterial::ShaderVariant* LitMaterial::getOrCreateVariant(int flag) {
    ...
    if (ok) {
        ...
        if (!linked) {
            ...
            if (logLen > 0) {
                ...
                LOGE("LitMaterial variant 0x%x link log: %s", flag, log.data());
            }
        }
    }
    ...
    v->baseColorLoc   = s->getUniformLocation("uBaseColor");
    v->lightPosLoc    = s->getUniformLocation("uLightPos");
    v->lightColorLoc  = s->getUniformLocation("uLightColor");
    v->camPosLoc      = s->getUniformLocation("uCameraPos");
    ...
    variants[flag] = v;
    LOGI("LitMaterial variant 0x%x: linked=%d", flag, v->linked ? 1 : 0);
    return v;
}

void LitMaterial::setTransform(const float* model, const float* view, const float* projection) {
    if (!initialized) return;
    ...
    glUniform3fv(v->baseColorLoc, 1, baseColor);
    glUniform3fv(v->lightPosLoc, 1, lightPos);
    glUniform3fv(v->lightColorLoc, 1, lightColor);
    if (camera) {
        float* camPos = camera->getPosition();
        glUniform3f(v->camPosLoc, camPos[0], camPos[1], camPos[2]);
    }
}
...
void LitMaterial::setBaseColor(float r, float g, float b, float a) {
    baseColor[0] = r; baseColor[1] = g; baseColor[2] = b; baseColor[3] = a;
}

void LitMaterial::setLightPos(float x, float y, float z) {
    lightPos[0] = x; lightPos[1] = y; lightPos[2] = z;
}

void LitMaterial::setLightColor(float r, float g, float b) {
    lightColor[0] = r; lightColor[1] = g; lightColor[2] = b;
}

const Material::VertexAttrib* LitMaterial::getVertexAttribs(int& count) {
...
std::string LitMaterial::buildVS(int flag) {
    std::string vs;
    vs.reserve(2048);
    vs = R"(#version 300 es
...
layout(location = 2) in vec2 aTexCoord;
    ...
    vs += R"(out vec3 vNormal;
out vec3 vWorldPos;
out vec2 vTexCoord;
flat out vec4 nowScreenPos;
flat out vec4 preScreenPos;
void main() {
    vec4 sPos = skinPos(aPosition);
    vec3 sNor = skinNormal(aNormal);
    vec4 worldPos = uModelMatrix * sPos;
    vWorldPos = worldPos.xyz;
    vNormal = normalize((uModelMatrix * vec4(sNor, 0.0)).xyz);
    vTexCoord = aTexCoord;
    ...
    vec4 prevWorldPos = uPrevModelMatrix * sPos;
...
std::string LitMaterial::buildFS(int flag) {
    std::string fs;
    fs.reserve(3072);
    fs = R"(#version 300 es
precision highp float;
...
void main() {
    ...
    if (flag & FLAG_BASE_COLOR_MAP) {
        // Sample base color map (assumed sRGB → linearise) and modulate.
        fs += "    albedo *= pow(texture(uBaseColorMap, vTexCoord).rgb, vec3(2.2));\n";
    }
    fs += R"(    vec3 diffuse = albedo * NdotL;
    vec3 ambient = albedo * 0.1;
    vec3 color = ambient + diffuse * uLightColor * atten;
)";
    if (flag & FLAG_SHADOW) {
